...
cat > tcl <<EOF
mol new raw.pdb
foreach ii {A B F G} jj {A B C D} {
  set sel [atomselect top "chain \$ii"]
  \$sel writepdb chains/PRO\$jj.pdb
}
quit
EOF
...
foreach ii {B} aa {23} bb {91} {
  segment PRO$ii {
    pdb chains/PRO$ii.pdb
    mutate 96 PRO
  }
  patch DISU PRO$ii:$aa PRO$ii:$bb
  coordpdb chains/PRO$ii.pdb PRO$ii
}

foreach ii {A C D} aa {23 22 22} bb {90 90 96} {
  segment PRO$ii {
    pdb chains/PRO$ii.pdb
  }
  patch DISU PRO$ii:$aa PRO$ii:$bb
