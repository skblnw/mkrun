...
\$sel writepdb chains/PROA.pdb
set sel [atomselect top "segname PROP"]
\$sel writepdb chains/PROP.pdb
quit
EOF
...
mol new prot.psf
mol addfile prot.pdb
mol new md.psf
mol addfile md.pdb
set sel1 [atomselect 0 all]
set sel2 [atomselect 1 "segname MEMB TIP3 IONS"]
set mol [::TopoTools::selections2mol "\$sel1 \$sel2"]
animate write psf vmd_solvate/ionized.psf \$mol
animate write pdb vmd_solvate/ionized.pdb \$mol
quit
EOF
$VMD -dispdev text -e tcl 
rm tcl
