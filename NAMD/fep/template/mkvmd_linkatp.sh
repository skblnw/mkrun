...
set sel [atomselect top "resname LIG and name C CA O N OXT"]
$sel set resid 1
...
segment LIG {
  pdb chains/LIG.pdb
  foreach name {C CA O N OXT} {
      pdbalias atom LIG ${name} ${name}X
  }
}
coordpdb chains/LIG.pdb LIG
coordpdb chains/ATP.pdb LIG

foreach ii {MG} {
  segment $ii {pdb chains/$ii.pdb}
  coordpdb chains/$ii.pdb $ii
}
