...
exec rm -r chains
exec mkdir chains
mol new ../../pdb2namd/solvate/ionized.psf
mol addfile ../../t1/output/md-1.restart.coor
foreach ii {A} {
    set sel [atomselect top "segname PRO$ii"]
    $sel writepdb chains/$ii.pdb
}
foreach ii {H} {
    set sel [atomselect top "segname PRO$ii and resid 1 to 120"]
    $sel writepdb chains/$ii.pdb
}
foreach ii {L} {
    set sel [atomselect top "segname PRO$ii and resid 1 to 108"]
    $sel writepdb chains/$ii.pdb
}
