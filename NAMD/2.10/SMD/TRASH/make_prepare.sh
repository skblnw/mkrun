...
ncpu=16
nheader=37
COMMAND="\$CHARMRUN +p\$NP2 ++ppn 15 ++scalable-start ++nodelist \$nodefile \$NAMD +setcpuaffinity +pemap 1-15 +commap 0"
...
if $mini; then
    ...
    do
        ...
        echo -e "$COMMAND mini-$ii.namd > log/mini-$ii.log\nwait\ndate" >> job-$jobname.pbs
    done
...
if $heat; then
    ...
    echo -e "$COMMAND heat.namd > log/heat.log\nwait\ndate" >> job-$jobname.pbs
fi
...
if $pre; then
    ...
    echo -e "$COMMAND pre-npt.namd > log/pre-npt.log\nwait\ndate" >> job-$jobname.pbs
...
if $cons; then
    ...
    do
        ...
        echo -e "$COMMAND cons-$NN.namd > log/cons-$NN.log\nwait\ndate" >> job-$jobname.pbs
        MM=$NN
    done
fi
...
if $smd; then
    for spd in 10 5 1; do
    jobname=smd$spd
    sed -e 's/^#PBS -N .*/#PBS -N '$jobname'/g' \
        -e 's/^#PBS -l nodes=.*/#PBS -l nodes='$nnode':ppn='$ncpu'/g' \
        template-job-pbs > job-$spd.pbs
    sed -i $nheader',$d' job-$spd.pbs

    previous=cons-1
    sed -e 's/^set INPUTNAME.*$/set INPUTNAME output\/'$previous'/g' \
        -e 's/^set OUTPUTNAME.*$/set OUTPUTNAME output\/smd'$spd'-1/g' \
        -e 's/^set FIXPDB.*$/set FIXPDB fix_memb/g' \
        -e 's/^COMmotion.*$/COMmotion no/g' \
        -e 's/^set PSWITCH.*$/set PSWITCH 0/g' \
        -e 's/^restartfreq.*$/restartfreq 1000/g' \
        -e 's/^dcdfreq.*$/dcdfreq 1000/g' \
        ...
        -e 's/set SEP.*$/set SEP 20/g' \
        -e 's/set SPEED_GLOB.*$/set SPEED_GLOB '$spd'/g' \
        template-smd-namd > smd$spd-1.namd
    echo -e "$COMMAND smd$spd-1.namd > log/smd$spd-1.log\nwait\ndate" >> job-$spd.pbs
    done
fi
