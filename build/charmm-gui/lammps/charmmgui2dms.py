...
class namdvel:
    def __init__(self, vfile):
        print vfile
        f = file(vfile, mode='r')
        self.natom = np.fromfile(f, dtype='i4', count=1)
        count = 0
    ...
    def generate_lammps(self, psf, crd, prm, boxsize, lammpscmap, lammpsangle, zcenter):
        ...
        for i in range(0, psf.atomnum):
            ...
            self.write("%12d %8d %6d %7.3f %16.10f %16.10f %16.10f #%s"%(i+1, int(psf.atom[i].resid), nonbondid+1,\
                    psf.atom[i].charge, crd.atom[i].coor[0], crd.atom[i].coor[1], crd.atom[i].coor[2], psf.atom[i].atomstr ))
...
def main():
    des= """convert a charmm system to Desmond dms file"""
    ...
    parser.add_argument('-crd', action="store", dest="crd", required=True,\
    ...
    if inputpsf.atomnum != inputcrd.atomnum:
        raise Exception("number of atoms in psf and crd do not match: %d in psf %d in crd"%(inputpsf.atomnum, inputcrd.atomnum))
