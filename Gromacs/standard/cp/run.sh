...
if $mini1; then
    ...
    gmx mdrun -pin on -ntomp 8 -v -s $prefix.tpr -deffnm output/$prefix
fi
...
if $heat; then
    ...
    gmx mdrun -pin on -ntomp 8 -v -s $prefix2.tpr -deffnm output/$prefix2
fi
...
if $md2; then
    ...
    gmx mdrun -pin on -ntomp 8 -v -s $prefix2.tpr -cpi output/$prefix1.cpt -deffnm output/$prefix2
fi
